...
- [ ] 1.1 Replace substring sentinel matching with whole-answer matching (lowercase, trim, strip trailing `.`/`!`) plus the raw `/^no\s/i` not-present rule (design D4). Verify with new `tests/import-parse.test.ts` cases: `"none-cemented"` → value, `"no-show"` → value, `"unclear-coated leather"` → value, `"no visible welt or stitching"` → absence on an absence-legal path, `"Cannot discern."` → unfilled. Existing value-state tests still pass.
