...
Parsed pass responses SHALL map to the sheet value states: a committed answer becomes a single value; a hedged answer naming alternatives becomes a choice-set proposal; an explicit not-present statement becomes an absence proposal only on paths where absence is defined; a cannot-discern statement leaves the field unfilled. A cannot-discern or not-present statement SHALL be recognized when it is the entire answer, or when the answer begins with a multi-word sentinel phrase (e.g. "cannot tell", "not present") followed by a space. A not-present statement SHALL also be recognized when the answer begins with the standalone word "no" followed by a space (e.g. "no visible welt or stitching"). Single-word sentinels (e.g. "none", "absent", "unclear") SHALL be recognized only as the entire answer. Matching ignores case and surrounding whitespace. A term that merely contains such a word (e.g. "none-cemented", "no-show", "unclear-coated leather") is an ordinary answer. For scale fields, an answer SHALL be accepted when it contains exactly one valid step reference for that field's scale, whatever decoration surrounds it. Any other value containing digits SHALL be rejected to unfilled with a note. Vocabulary answers outside the seeded terms SHALL be accepted as custom values, normalized to the house term format.
