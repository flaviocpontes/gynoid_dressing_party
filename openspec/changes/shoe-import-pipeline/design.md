...
### D5: Battery execution — one action, per-pass persistence, sequential requests
A single server action `runBatteryAction(runId)` fires the selected passes; each pass independently writes its row as soon as it settles (partial progress visible), failures recorded as pass rows with empty proposals rather than aborting the run, and failed passes are retryable (re-running the battery only executes missing/failed keys). Family pass is its own action run first. Vibe runs skip pass 0's image interrogation: one full-template text call (the battery collapsed to a single pass, since decomposition earns its keep on vision), then the same review surface.
*Implementation constraint (user directive, supersedes the spec's "run in parallel" wording): the Lemonade server gets exactly one inference request at a time — the single LLM slot wedges under concurrent bursts. The client also retries once with backoff on transport failures.*
